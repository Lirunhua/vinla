...
# Calculating the mutual information between two attributes.
...
class DataSet:
    ...
    def __init__(self, data):
        ...
        self.each_attrname_array = pickle.load(open('./pickleFile/' + Global_V.TESTFILE+'_each_attrVal_array.pkl', 'rb'))
    ...
    def splitdataset(self, splitratio, startline):
        ...
        trainset = []
        copy = list(self.data)  # 注意这种拷贝方式存在的问题

        while len(trainset) < trainsize:
            if len(copy) <= startline:
                startline = 0
            trainset.append(copy.pop(startline))
        ...
        for a in range(0, self.getNoAttr(), 1):
            ...
            for v in range(0, self.getCountOfAttr_i(a), 1):
                for y in range(0, self.getCountOfAttr_i(-1)):
                    ...
                    if avyCount:
                        m += (avyCount /totalCount) * math.log(avyCount/((self.getXCount(a, v)/totalCount)* self.getClassCount(y)),2)
            mi.append(m)
        return mi
                ...
                for v1 in range(0, self.getCountOfAttr_i(x1)):
                    for v2 in range(0, self.getCountOfAttr_i(x2)):
                        for y in range(0, self.getCountOfAttr_i(-1)):
                            ...
                            if x1x2y:
                                m += (x1x2y / self.totalcount) * math.log(self.getClassCount(y) * x1x2y / ((self.getXYCount(x1, v1, y) * self.getXYCount(x2, v2, y))),2)
                cmi[x1][x2] = m
                cmi[x2][x1] = m
        return cmi
